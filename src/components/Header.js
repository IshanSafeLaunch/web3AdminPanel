        ...
        <header className="header-content" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px' }}>
          ...
          {!account && (
            <button
              ...
              style={{ backgroundColor: 'lightblue', border: 'none', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer', outline: 'none' }}
              onClick={connectMetamask}
            >
              Connect Metamask
            </button>
          )}
          {account && <h3>Wallet address: {account}</h3>}
        </header>
      </div>
      <Head accounts={account} />
      {/* <UploadForm accounts={account} /> */}
      {/* <UpploadForm accounts={account} /> */}
    </div>
  );
}
